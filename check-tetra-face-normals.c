...
DEFINE_ON_DEMAND(check_tetra_face_normals)
{
    ...
    /* Tetrahedron cell face connectivity list given in section 6.1.2.3
    of the Fluent User's Guide. Changed to use 0 as the starting node
    index. */
    ...
    cxboolean are_all_vectors_ok = TRUE;
    ...
    thread_loop_c(cell_thread, domain) {
        if (THREAD_N_ELEMENTS_INT(cell_thread) > 0) {
            begin_c_loop_int(cell, cell_thread) {
                ...
                which are identified as having four nodes. */
                if (C_NNODES(cell, cell_thread) == 4) {
                    ...
                    c_face_loop(cell, cell_thread, face_index) {
                        ...
                        /* Here I am using the face connectivity
                        described in the documentation instead of using
                        the nodes of the face themselves (in the order
                        defined on the face structure). That is because
                        Fluent does not duplicate face data in memory,
                        so there will always be some cells with normals
                        pointing inside the cell (because the face was
                        created using the neighbor data, pointing out of
                        the neighbor cell). */
                        NV_VV(
                        ...
                        /* If the face normal is pointed out of the
                        cell, the dot product of the normal vector
                        calculated above and the vector from the cell
                        centroid to the face centroid will be positive.
                        Otherwise there is an inconsistency on the
                        connectivity. */
